...
**Show Personalized/OEM email address everywhere**  OEM 2.0 partners can request a channel-level configuration that lets Acrobat Sign display a user’s actual company email address in the UI instead of the partner-managed email value used for provisioning and identity management.  This behavior is **not** enabled by default.  When enabled, emailAlias becomes required during user creation and must contain the user’s actual company email address. 
