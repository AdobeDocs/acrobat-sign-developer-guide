...
Repeating a create request with the same email in the same account returns `201 Created` and the existing `userId`. \<br/\>A `409 USER_ALREADY_EXISTS` error is returned when the email conflicts with a user in a different account.\<br/\>If the target account has not finished provisioning, this call returns a `409 ACCOUNT_NOT_READY` error: "Account provisioning is still in progress. Please retry."
